...
def test_approve_tokens(maker, taker, weth, zrx, erc20proxy):
    allowance = 2 ** 256 - 1
    weth.approve(erc20proxy, allowance, {'from': maker.account})
    weth.approve(erc20proxy, allowance, {'from': taker.account})
    assert weth.allowance(maker.account, erc20proxy) == allowance and \
        weth.allowance(taker.account, erc20proxy) == allowance
    zrx.approve(erc20proxy, allowance, {'from': maker.account})
    zrx.approve(erc20proxy, allowance, {'from': taker.account})
    assert zrx.allowance(maker.account, erc20proxy) == allowance and \
        zrx.allowance(taker.account, erc20proxy) == allowance
...
def test_fillOrder(maker, taker, zrx, weth, exchange):
    ...
    order, sig = maker.make_zx_order(zrx, 801, weth, 1)
    getcontext().prec = 23
    fill_amt = Decimal(20)/182.76        # buy $20 worth of weth, worth of zrx
